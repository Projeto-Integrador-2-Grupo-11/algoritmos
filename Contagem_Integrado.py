...
while True:
    ...
    cliente_prod = MongoClient('mongodb://localhost:27017/orange_classification')
    db_prod   = cliente_prod.orange_classification
    coll_quantity = db_prod.quantity_oranges
    ultimo_quantity_oranges = coll_quantity.find({}).sort('_id', -1).limit(1)
    for doc in ultimo_quantity_oranges:
        lote = doc['batch']
    ...
    if distancia_pequena <= 50 and distancia_pequena > 0:
        ...
        coll_quantity.update_one({ 'batch': lote }, {"$set": { "small_oranges": laranja_pequena }})
    if distancia_media <= 50 and distancia_media > 0:
        laranja_media = laranja_media + 1
        coll_quantity.update_one({ 'batch': lote }, {"$set": { "medium_oranges": laranja_media }})
    if distancia_grande <= 50 and distancia_grande >0:
        laranja_grande = laranja_grande + 1
        coll_quantity.update_one({ 'batch': lote }, {"$set": { "large_oranges": laranja_grande }})
    else:
